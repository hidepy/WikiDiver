...
//(function(){
{
    ...
    ons.ready(function () {
        ...
        // キャッシュ設定
        var m_his_cache = storage_manager_settings.getItem(SETTING_TYPE.HISTORY_CACHE);
        if (!m_his_cache) {
            m_his_cache = 1;
            storage_manager_settings.saveItem2Storage(SETTING_TYPE.HISTORY_CACHE, m_his_cache);
        }
        // フォントサイズ
        var m_font_size = storage_manager_settings.getItem(SETTING_TYPE.FONT_SIZE);
        if (!m_font_size) {
            m_font_size = 100;
    ...
    module.controller("DetailController", function ($scope, $sce, $compile, popoverSharingService) {
        ...
        if (_args.onTransitionEnd && _args.onTransitionEnd.need_onload_search) {
            if (false) {
                ...
                // キャッシュデータがあれば格納
                var cached_data = storage_manager_history.getItem(title);
